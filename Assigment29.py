...
def CheckArray(NumberList,Average):
    ...
    for i in range(0,len(NumberList)):
        Len=len(NumberList)-(i+1)

        while Len>0:
            Avg=NumberList[i]
            TempStack=[]
            TempStack.append(NumberList[i])

            for j in range(i+1,Len):

                TempStack.append(NumberList[j])

            FinalAvg=statistics.mean(TempStack)
            if(FinalAvg==Average):
                NumberList2=NumberList.copy()

                for abc in range(0,len(TempStack)):
                    NumberList2.remove(TempStack[abc])
                Averageofsecond=statistics.mean(NumberList2)

                if(Averageofsecond==Average):
                    print(TempStack)
                    print(NumberList2)
                    return True
                    break
